  ...
  <div class="container">
    <div class="row text-center">
      ...
      <h2 class="mb-3">QR को स्कैन करके Add Money करें</h2>
        ...
        <form id="payment" method="post" action="{{ URL::to('user/upload-reciept') }}" enctype="multipart/form-data">
          @csrf
    ...
    <div class="helpline mt-4">
        Helpline Number - <span class="helpline-number">+91 {{$upi->help_line_number}}</span>
      </div>
      ...
      @if($paymentInstruction->file == null)
      ...
      <p style='margin-left: 21px; margin-top: 42px;'>{{$paymentInstruction->description}}</p>
      </div>
    ...
    @else
      ...
      <video width="320" height="240" controls>
        <source src="{{URL::asset('backend/uploads/payment_instruction/' . $paymentInstruction->file)}}"
        type="video/mp4">
      </video>
      </div>
    @endif
